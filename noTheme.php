        ...
        <section>
		  ...
		  $start_location = 1;
			 ...
			 // $varLoadLimit = 10;
			  ...
			  $sql = "SELECT count(article_id) AS 'count' FROM `article` WHERE NOT article_id IN (SELECT DISTINCT article FROM article_themes_belongs_to WHERE NOT theme = 0)";
			  $result = mysql_query($sql);
			  $row = mysql_fetch_object($result);
			   $count = $row->count;
			   
			   $sql = "SELECT count(article_id) AS 'count' FROM `article`";
			  $result = mysql_query($sql);
			  $row = mysql_fetch_object($result);
			   $count2 = $row->count;
			  
			  $quote=  floor($count/$count2*10000)/100;
			  $qutoez = 100-$quote;
			  
			  	$article = '
			<article class="timeline-article">
			  
			  <h1>No Theme Timeline</h1>
			  <p>Diese Artikel konnten keinem Thema zugeordnet werden.</p>
			    <p>'.$count.' ('.$quote.'%) von '.$count2.' Artikeln wurden <b>nicht</b> zugeordnet</p>
				 <p>Zugeordnet: '. $qutoez.'% </p>
			...
			echo $article;
			    ...
			    $sql1 = "SELECT * FROM `article` WHERE NOT article_id IN (SELECT DISTINCT article FROM article_themes_belongs_to WHERE NOT theme = 0)";
			...
			<article class="timeline-article">
			  ...
			  <div class="article-subbox">
				<div class="article-origin">
				  Quelle: <a  href="redirect.php?id=' . $art_id . '" title="' . $strippedlink . '" target="_new">' . $strippedlink_cropped . '</a>
				</div>
