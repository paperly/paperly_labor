            ...
            $(document).ready(function() {
                ...
                $('.fancybox').fancybox({
                    ...
                    beforeClose: function() {
                        parent.history.pushState(null, 'paperly Artikel', 'http://www.paperly.de/index.php');
                    }
                });
            });
...
while ($row = mysql_fetch_object($result44)) {
    ...
    $sql2 = "SELECT * FROM  user WHERE user_id = '$creator_id';";
    ...
    $text = stripArticleText(strip_tags($row->article_text), $articleTextLength, $art_id);
    // format link
    $bild = $row->image;
    ...
    $bild_html = "";
    if (empty($link)) {
        $bild_html = "<a class='fancybox fancybox.iframe' href='/articleopen/" . $art_id . "' target='_new' ><img src='upload/timeline/$bild' width='300' height='95' alt='paperly'></a>";
    } else {
                                      ...
                                      <div class="article-subbox">
                                           ...
                                           ' . $quellen_text . '
                                            ...
                                            <div class="article-post-time">
                                             <a href="/'.$creator_name.'">'.$creator_name.'</a> ' . $time . '
                                            </div>
